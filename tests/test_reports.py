...
async def test_reports_totals(client, auth_headers, db):
    ...
    make_alert(db, station_id="STN-042", status=AlertStatus.open)
    make_alert(db, station_id="STN-042", status=AlertStatus.resolved)
    make_alert(db, station_id="STN-043", status=AlertStatus.open)
...
async def test_reports_by_station_breakdown(client, auth_headers, db):
    ...
    assert "STN-042" in by_station
    assert "STN-043" in by_station
    assert by_station["STN-042"]["total"] == 2
    assert by_station["STN-042"]["open"] == 1
    assert by_station["STN-042"]["resolved"] == 1
    assert by_station["STN-043"]["total"] == 1
    assert by_station["STN-043"]["open"] == 1
...
async def test_reports_by_station_sorted_by_total_descending(client, auth_headers, db):
    ...
    make_alert(db, station_id="STN-001")
    make_alert(db, station_id="STN-002")
    make_alert(db, station_id="STN-002")
    make_alert(db, station_id="STN-002")
    ...
    assert data["by_station"][0]["station_id"] == "STN-002"
    assert data["by_station"][1]["station_id"] == "STN-001"
...
async def test_avg_resolution_minutes_for_station_filter(client, auth_headers, db):
    ...
    alert = make_alert(db, station_id="STN-042", status=AlertStatus.resolved)
    ...
    response = await client.get(
        "/reports/alerts?station_id=STN-042", headers=auth_headers
    )
    data = response.json()
...
async def test_filter_by_station_id(client, auth_headers, db):
    ...
    make_alert(db, station_id="STN-042")
    make_alert(db, station_id="STN-043")
...
async def test_filter_by_station_id_no_results(client, auth_headers, db):
    ...
    response = await client.get(
        "/reports/alerts?station_id=STN-999", headers=auth_headers
    )
    assert response.status_code == 200
    data = response.json()
    assert data["total"] == 0
...
async def test_filter_by_from_date(client, auth_headers, db):
    ...
    old_alert = make_alert(db, station_id="STN-042")
    old_alert.created_at = datetime(2026, 1, 1, tzinfo=timezone.utc)
    db.commit()

    recent_alert = make_alert(db, station_id="STN-042")
...
async def test_filter_by_date_range(client, auth_headers, db):
    ...
    for month in [1, 3, 6]:
        alert = make_alert(db, station_id="STN-042")
