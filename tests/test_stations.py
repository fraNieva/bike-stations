...
async def test_list_stations_shows_current_status(client, auth_headers, db):
    ...
    make_event(db, station_id="STN-042", is_charging=False)
    make_event(db, station_id="STN-042", is_charging=True)
    make_event(db, station_id="STN-043", is_charging=False)

    response = await client.get("/stations", headers=auth_headers)
    assert response.status_code == 200
    data = response.json()

    bcn_042 = next(s for s in data if s["station_id"] == "STN-042")
    bcn_043 = next(s for s in data if s["station_id"] == "STN-043")
...
async def test_get_station_detail(client, auth_headers, db):
    ...
    for i in range(12):
        make_event(db, station_id="STN-042", is_charging=i % 2 == 0)

    response = await client.get("/stations/STN-042", headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
...
async def test_get_station_not_found(client, auth_headers):
    ...
    response = await client.get("/stations/STN-999", headers=auth_headers)
